...
func (h *Handler) ServeProxy(w http.ResponseWriter, r *http.Request) {
	...
	// Model aliasing: rewrite client model to upstream model before summarization
	if h.Cfg.ClientModelAlias != "" && h.Cfg.UpstreamModel != "" {
		body = RewriteModel(body, h.Cfg.ClientModelAlias, h.Cfg.UpstreamModel)
	}
	...
	// When AR_API_KEY is set, use it for upstream auth instead of client's auth
	// Otherwise, preserve passthrough behavior for backward compatibility
	if h.Cfg.ARAPIKey != "" {
		if strings.HasPrefix(path, "/v1/messages") {
			upstreamHeaders.Set("x-api-key", h.Cfg.ARAPIKey)
		} else {
			upstreamHeaders.Set("Authorization", "Bearer "+h.Cfg.ARAPIKey)
		}
	} else {
...
func (h *Handler) streamSSE(w http.ResponseWriter, r *http.Request, resp *http.Response, model string, streamFormat StreamFormat, ctx context.Context) {
	...
	// Add model aliasing if configured
	if h.Cfg.ClientModelAlias != "" && h.Cfg.UpstreamModel != "" {
		opts.ModelOld = h.Cfg.UpstreamModel
		opts.ModelNew = h.Cfg.ClientModelAlias
	}
	res := PumpSSE(ctx, w, resp.Body, opts)
...
func (h *Handler) copyBody(w http.ResponseWriter, resp *http.Response, model string, respStart time.Time) {
	...
	_ = resp.Body.Close()
	// Rewrite model in response if configured
	if h.Cfg.ClientModelAlias != "" && h.Cfg.UpstreamModel != "" {
		raw = ResponseRewriteModel(raw, h.Cfg.UpstreamModel, h.Cfg.ClientModelAlias)
	}
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Access-Control-Allow-Origin", "*")
	_, _ = w.Write(raw)
...
func (h *Handler) writeFull(w http.ResponseWriter, status int, headers http.Header, body []byte) {
	// Rewrite model in response if configured
	if h.Cfg.ClientModelAlias != "" && h.Cfg.UpstreamModel != "" {
		body = ResponseRewriteModel(body, h.Cfg.UpstreamModel, h.Cfg.ClientModelAlias)
	}
	for k, vv := range headers {
		for _, v := range vv {
			w.Header().Add(k, v)
